...
def calculate_metrics_third_column(input_file, output_file, label_column, prediction_column, group_column, thresholds_number, thresholds_range, positive_value):
    ...
    if label_column not in data.columns or prediction_column not in data.columns or group_column not in data.columns:
        raise ValueError(f"Input file must contain columns: {label_column}, {prediction_column}, and {group_column}. Found: {data.columns.tolist()}")
    ...
    for group_value in data[group_column].unique():
        group_data = data[data[group_column] == group_value].copy()
        group_data = group_data.sort_values(prediction_column)
        
        cumulative_benign = pd.Series([False] * len(group_data), index=group_data.index)
        ...
        for threshold in thresholds:
            ...
            group_data["binary_prediction"] = np.where(cumulative_benign, "benign", "pathogenic")

            # Calculate metrics
            tn, fp, fn, tp = confusion_matrix(group_data[label_column], group_data['binary_prediction'], labels=['benign', 'pathogenic']).ravel()
            precision = precision_score(group_data[label_column], group_data['binary_prediction'], pos_label='pathogenic', zero_division=0)
            recall = recall_score(group_data[label_column], group_data['binary_prediction'], pos_label='pathogenic', zero_division=0)
            f1 = f1_score(group_data[label_column], group_data['binary_prediction'], pos_label='pathogenic', zero_division=0)
            f2 = (5 * precision * recall) / (4 * precision + recall) if (precision + recall) > 0 else 0
            accuracy = accuracy_score(group_data[label_column], group_data['binary_prediction'])
            balanced_acc = balanced_accuracy_score(group_data[label_column], group_data['binary_prediction'])
...
if __name__ == "__main__":
    calculate_metrics_third_column()